...
- モーダルを開くたびに画像の実効 `opacity` を1にリセットするようにし、再表示時に暗くなる問題を解消しました。
- Escapeキー押下時にモーダルが正しく閉じない不具合（タイポ）を修正しました。

## NFT詳細モーダルのレイアウト調整・年度表示追加

情報の表示順序を整理し、新たに「年度（Season）」項目を追加しました。

### 変更点
- **表示順序の変更**: 以下の順序で情報を整理しました。
    1. **Token ID**
    2. **座標 (X, Y)**（2列表示に整理）
    3. **Total KSP**
    4. **Season Rank**
    5. **Season (年度)**
- **JavaScript拡張**: `openTokenModal` 関数に第6引数として年度データ（Season）を追加。
- **データ連携**: マップ上のピン、所有アセットリストの各要素から正しい年度データがモーダルに渡されるよう修正。
